...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Bullion Vault bullion-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  bullion-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded Bullion transaction") + "\n" +
                               "  bullion-tx [options] -create [commands]   " + _("Create hex-encoded Bullion transaction") + "\n" +
                               "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        mergedTx.nTime = GetAdjustedTime();
        mergedTx.UpdateHash();
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mergedTx.vout.size()))
            //SignSignature(keystore, prevPubKey, mergedTx, i, nHashType);
            SignSignature2(keystore, prevPubKey, mergedTx, i, nHashType);
        ...
        if (!VerifyScript2(txin.scriptSig, prevPubKey, mergedTx, i, true, 0))
            fComplete = false;
    }
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded Bullion transaction
