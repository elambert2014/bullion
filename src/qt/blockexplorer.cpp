...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CBX, nValue);
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValue(0, pBlock->nBits, pBlock->nTime, pBlock->nHeight-1, pBlock->nMoneySupply, Fees, pBlock->IsProofOfStake());
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (bullion.conf).");
            QMessageBox::warning(this, "Bullion Vault Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
