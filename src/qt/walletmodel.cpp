...
void WalletModel::pollBalanceChanged()
{
    ...
    if (fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks || nObfuscationRounds != cachedObfuscationRounds /*|| cachedTxLocks != nCompleteTXLocks*/) {
        ...
        emit numBlocksChanged(cachedNumBlocks);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered Bullion address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bullion:URI (bullion:XyZ...?message=example)
            {
...
int64_t WalletModel::getPoSPInstantProfitability(float& yield)
{
    if (getEncryptionStatus() == WalletModel::EncryptionStatus::Locked || 
        getBalance() + getUnconfirmedBalance() == 0 || !masternodeSync.IsBlockchainSynced())
    {
        yield = 0;
        return -1;
    }

    return wallet->GetPoSPInstantProfitability(chainActive.Tip(), yield);
}