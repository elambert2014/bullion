...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Bullion users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Bullion Vault " + FormatFullVersion();
...
void ThreadOpenConnections()
{
    ...
    // Add well-known trusted peers
    {
        const string strWellKnownNodes[] = { "node1.bullion.one", "node2.bullion.one", "node3.bullion.one" };
        LOCK(cs_vAddedNodes);
        BOOST_FOREACH (string strNode, strWellKnownNodes) {
            CAddress addr;
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bullion Vault is probably already running."), addrBind.ToString());
        else
...
void static Discover(boost::thread_group& threadGroup)
{
    ...
    // Don't use external IPv4 discovery, when -onlynet doesn't include IPV4
    if (!IsLimited(NET_IPV4))
    {
        threadGroup.create_thread(boost::bind(&TraceThread<void (*)()>, "addcon", &ThreadGetMyExternalIP));
        MilliSleep(1000);
    }
}
