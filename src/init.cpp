...
#include "config/bullion-config.h"
#endif
...
class CBlockchainSnapshotInstaller
{
private:
    boost::filesystem::path archive;
    char errorBuffer[CURL_ERROR_SIZE] = {'\0'};
    const char* bootstrapUrl = "http://data.bullion.one/v30/bootstrap.zip";

    static int CopyFileData(struct archive *ar, struct archive *aw)
    {
        int r;
        const void *buff;
        size_t size;
        ...
        for (;;) {
            ...
            if (r == ARCHIVE_EOF)
                return (ARCHIVE_OK);
            if (r != ARCHIVE_OK) {
                LogPrintf("archive_read_data_block(): %s [%d]\n", archive_error_string(ar), archive_errno(ar));
                return (r);
            }
            r = archive_write_data_block(aw, buff, size, offset);
            if (r != ARCHIVE_OK) {
                LogPrintf("archive_write_data_block(): %s [%d]\n", archive_error_string(aw), archive_errno(aw));
                return (r);
            }
        }
    }

    void DeleteSnapshotArchive()
    {
...
public:
    CBlockchainSnapshotInstaller()
    {
        archive = GetTempPath() / "bootstrap.zip";
    }
    ...
    static int DownloadProgressCallback(void *clientp, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
    {
        ...
        if (lastPercentage != percentage) {
            ...
            uiInterface.ShowProgress(_("Downloading blockchain snapshot..."), percentage);
        }
        return 0;
    }

    int DownloadBlockchainSnapshot()
    {
        CURL *conn = NULL;
        CURLcode code;
        ...
        code = curl_easy_setopt(conn, CURLOPT_URL, bootstrapUrl);
        if (code != CURLE_OK) {
        ...
        FILE* bootstrapFile = fopen(archive.string().c_str(), "ab");

        code = curl_easy_setopt(conn, CURLOPT_WRITEDATA, bootstrapFile);
        if (code != CURLE_OK) {
        ...
        code = curl_easy_setopt(conn, CURLOPT_XFERINFOFUNCTION, DownloadProgressCallback);
        if (code != CURLE_OK) {
            LogPrintf("Failed to set xfer info function [%s]\n", errorBuffer);
        }

        code = curl_easy_setopt(conn, CURLOPT_NOPROGRESS, 0);
        if (code != CURLE_OK) {
            LogPrintf("Failed to set progress feedback activation [%s]\n", errorBuffer);
        }
    ...
    int ExtractBlockchainSnapshot()
    {
        uiInterface.InitMessage(_("Extracting blockchain data..."));

        // Clean all blockchain data, including that of previous wallet versions
        boost::filesystem::remove_all(GetDataDir() / "blk*.dat");
        ...
        if ((r = archive_read_open_filename(a, archive.string().c_str(), 10240))) {
            LogPrintf("Failed to open '%s' [%d]\n", archive.string().c_str(), r);
            return -1;
        }
        ...
        for (;;) {
            ...
            if (r == ARCHIVE_EOF)
                break;
            if (r != ARCHIVE_OK) {
                LogPrintf("Failed to read next header [%d]\n", r);
                ret = -2;
                break;
            }
            archive_entry_set_pathname(entry, (GetDataDir() / archive_entry_pathname(entry)).string().c_str()); 
            ...
            if (r != ARCHIVE_OK) {
                ...
                r = CopyFileData(a, ext);
                if (r != ARCHIVE_OK)
                    ret = -4;
                r = archive_write_finish_entry(ext);
                if (r != ARCHIVE_OK) {
                    LogPrintf("archive_write_finish_entry(): %s [%d]\n", archive_error_string(ext), archive_errno(ext));
                    ret = -5;
                }
            }
        }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bullion.conf"));
    ...
    strUsage += HelpMessageOpt("-bootstrap", _("Bootstrap blockchain by downloading and applying most recent snapshot") + " " + _("on startup"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bulliond.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7695, 51474));
    ...
    strUsage += HelpMessageOpt("-fastindex", strprintf(_("Fast load of block index by reusing cached block hash (default: %u)"), 1));
    strUsage += HelpMessageOpt("-keypool=<n>", strprintf(_("Set key pool size to <n> (default: %u, minimum: %u)"), 100, 100));
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CBX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CBX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-usechangeaddresses", strprintf(_("Enable the use of automatically generated change addresses (default: %u)"), 0));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, bullion, (obfuscation, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CBX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Bullion specific functionality (Masternodes, Fungibility, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8395, 18395));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2013-%i The Bullion Foundation"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Vault is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // -bootstrap on an old wallet requires a rescan
    if (GetBoolArg("-bootstrap", false) && !GetBoolArg("-bootstrapnew", false)) {
        // Rescan if bootstrapping on old wallet
    ...
    // Record Vault version in coinbase
    char pszCoinbaseFlags[50];
    sprintf(pszCoinbaseFlags, "/%d/", CLIENT_VERSION / 100);    // We don't care about build number
    COINBASE_FLAGS << std::vector<unsigned char>(pszCoinbaseFlags, pszCoinbaseFlags + strlen(pszCoinbaseFlags));
    ...
    fUseChangeAddresses = GetArg("-usechangeaddresses", false);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bullion Vault is shutting down."));
...
#endif
    // Make sure only a single Vault process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bullion Vault is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bullion Vault version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    // ********************************************************* Step 6.5: load blockchain snapshot
    if (GetBoolArg("-bootstrap", false)) {
        LogPrintf("Bootstrapping blockchain with latest snapshot...\n");
        CBlockchainSnapshotInstaller snapshotInstaller;
        if (snapshotInstaller.DownloadBlockchainSnapshot()) {
            uiInterface.InitMessage(_("Download failed!"));
            MilliSleep(4000);   // Allow user to read
        }
        else if (snapshotInstaller.ExtractBlockchainSnapshot()) {
            uiInterface.InitMessage(_("Snapshot extraction failed!"));
            MilliSleep(4000);   // Allow user to read
        }
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bullion Vault") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Vault to complete") << "\n";
