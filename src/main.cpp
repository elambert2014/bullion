...
#if defined(NDEBUG)
#error "CBX cannot be compiled without assertions."
#endif
...
int nStakeTargetSpacing = 65;
bool fAlerts = DEFAULT_ALERTS;
unsigned int nStakeMaxAge = -1; // stake age of full weight
bool fUseFastIndex = true; // cache scrypt hashes to disk
bool fUseChangeAddresses = false; // send change to newly created addresses
...
static CBigNum bnProofOfStakeLimitV2(~uint256(0) >> 26);


static CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);
static CBigNum bnProofOfStakeLimitTestNet(~uint256(0) >> 20);

/** Fees smaller than this (in BULLS/uCBX) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Bullion Signed Message:\n";
...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (!EvalScript2(stack, tx.vin[i].scriptSig, tx, i, 0))
            return false;
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    ...
    BOOST_FOREACH (const CTxIn& txin, tx.vin) {
        if (txin.prevout.IsNull())
            continue; // no previous transaction
...
int64_t GetPoSReward(int64_t nCoinAge, unsigned int nBits, unsigned int nTime, bool fTestNet){
...
int64_t GetPoSPRewardV1(int64_t nMoneySupply){
    return (nMoneySupply*0.02)/485169;
}

int64_t GetPoSPRewardV2(int64_t nMoneySupply, int nFees){
...
int64_t GetBlockValue(int64_t nCoinAge, unsigned int nBits, unsigned int nTime, unsigned int nHeight, int64_t nMoneySupply, int nFees, bool fPoS)
{
    if(fPoS){
        ...
        return GetProofOfWorkReward(nHeight);
    }
}

int64_t GetMasternodePayment(int nHeight, int64_t nMoneySupply, int64_t nFees)
{
    if(nHeight <= HARDFORK_HEIGHTV5)
        return 0;
...
unsigned int static GetNextTargetRequiredPoSPV3(const CBlockIndex* pindexLast) {
    // This function improves trend towards the intended block time of 65 seconds
    static const int64_t nTargetSpacingWorkMaxPoSPV3 = 39 * 60;
    ...
    int64_t nTargetSpacing = GetStakeTargetSpacing(pindexLast->nTime);
    int64_t nInterval = nTargetSpacingWorkMaxPoSPV3 / nTargetSpacing;
    int64_t nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    if (nActualSpacing < 0) {
        nActualSpacing = 1;
    } else if (nActualSpacing > nTargetSpacingWorkMaxPoSPV3 / 2) {
        nActualSpacing = nTargetSpacingWorkMaxPoSPV3 / 2;
    }
    ...
    bnNew *= (nInterval * nTargetSpacing + nActualSpacing);
...
unsigned int static GetNextTargetRequiredPoSPV2(const CBlockIndex* pindexLast) {
    static const int64_t nTargetSpacingWorkMaxPoSPV2 = 16 * 60;
    ...
    int64_t nTargetSpacing = GetStakeTargetSpacing(pindexLast->nTime);
    int64_t nInterval = nTargetSpacingWorkMaxPoSPV2 / nTargetSpacing;
    int64_t nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    if (nActualSpacing < 0) {
        nActualSpacing = 1;
    } else if (nActualSpacing > nTargetSpacingWorkMaxPoSPV2) {
        nActualSpacing = nTargetSpacingWorkMaxPoSPV2;
    }
...
unsigned int static GetNextTargetRequiredPoSP(const CBlockIndex* pindexLast) {
    static const int64_t nTargetSpacingWorkMaxPoSP = 16 * 60;
    ...
    if(pindexLast->nHeight >= 2303458){
        return (unsigned int) -1;
    }

    if(pindexLast->nHeight == HARDFORK_HEIGHTV2) 
        return 487245310; // Hardcoded way to get wallet unstuck
    ...
    if(pindexLast->nTime >= HARDFORK_TIMEV3) {
        if(nActualSpacing < 0) {
            nActualSpacing = 1;
        }else if(nActualSpacing > nTargetSpacingWorkMaxPoSP) {
            nActualSpacing = nTargetSpacingWorkMaxPoSP;
        }
    }else{
        if(pindexLast->nHeight >= HARDFORK_HEIGHTV2){
            if(nActualSpacing < 0){
    ...
    int64_t nTargetSpacing = GetStakeTargetSpacing(pindexLast->nTime);
    int64_t nInterval = nTargetSpacingWorkMaxPoSP / nTargetSpacing;
    bnNew *= ((nInterval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
    bnNew /= ((nInterval + 1) * nTargetSpacing);
...
unsigned int static GetNextTargetRequiredHybrid(const CBlockIndex* pindexLast, bool fProofOfStake){
    static const int64_t OLD_nTargetSpacingWorkMax = 12 * 60;
    ...
    int64_t nTargetSpacing = fProofOfStake ? GetStakeTargetSpacing(pindexLast->nTime) : min(OLD_nTargetSpacingWorkMax, (int64_t) GetStakeTargetSpacing(pindexLast->nTime) * (1 + pindexLast->nHeight - pindexPrev->nHeight));
...
{
    if(pindexLast->nHeight > HARDFORK_HEIGHTV5 && fProofOfStake)
        return GetNextTargetRequiredPoSPV3(pindexLast);

    if(pindexLast->nHeight >= HARDFORK_HEIGHTV4 && pindexLast->nHeight <= HARDFORK_HEIGHTV5 && fProofOfStake)
...
unsigned int GetNextTargetRequiredBPN(const CBlockIndex* pindexLast)
{
    // This function improves trend towards the intended block time of 65 seconds
    static const int64_t nTargetSpacingWorkMaxPoSPV3 = 39 * 60;
...
bool CScriptCheck::operator()()
{
    ...
    if (!VerifyScript2(scriptSig, scriptPubKey, *ptxTo, nIn, true, 0)) {
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            unsigned int nPrevTime = coins->nTime;
            if (!nPrevTime) // coins->nTime should have a value only if previous tx is in same block
            {
                CTransaction txPrev;
                uint256 blockPrev;
                if (!GetTransaction(prevout.hash, txPrev, blockPrev, true))
                    return state.Invalid(
                        error("CheckInputs() : %s prev tx %s index entry not found", tx.GetHash().ToString().c_str(),  prevout.hash.ToString().c_str()),
                        REJECT_INVALID, "bad-txns-unknown-prev-tx");
                nPrevTime = txPrev.nTime;
            }

            // ppcoin: check transaction timestamp
            if (nPrevTime > tx.nTime)
                return state.DoS(100, error("CheckInputs() : transaction timestamp earlier than input transaction"),
                    REJECT_INVALID, "bad-txns-inputvalues-older");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nTime >= END_POW_TIME && block.IsProofOfWork())
...
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, CBlock* pblock)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        // Connect new blocks.
        for (unsigned int i = vpindexToConnect.size() - 1; vpindexToConnect.size() > i; --i) {
            CBlockIndex * pindexConnect = vpindexToConnect[i];
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (!pindexNew->SetStakeEntropyBit(block.ComputeStakeEntropyBit(pindexNew->nHeight)))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.GetBlockTime() > (int64_t)block.vtx[0].nTime + nMaxClockDrift)
        return state.DoS(50, error("CheckBlock() : coinbase timestamp is too early"),
            REJECT_INVALID, "bad-cb-timestamp");
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH (const PAIRTYPE(int, CBlockIndex*) & item, vSortedByHeight) {
        ...
        // CBX: calculate stake modifier checksum
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if(pfrom->nVersion > 0 && pfrom->nVersion < HARDFORK_PROTOCOL_VERSIONV5 && chainActive.Tip()->nHeight >= HARDFORK_HEIGHTV5)
    {
        // disconnect from peers older than this proto version
        LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, pfrom->nVersion);
        pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
            strprintf("Version must be %d or greater", HARDFORK_PROTOCOL_VERSIONV5));
    ...
    if (strCommand == "version") {
        ...
        if(pfrom->nVersion < HARDFORK_PROTOCOL_VERSIONV5 && chainActive.Tip()->nHeight > HARDFORK_HEIGHTV5)
        {
            // disconnect from peers older than this proto version
            LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, pfrom->nVersion);
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
        ...
        if (pfrom->nVersion < HARDFORK_PROTOCOL_VERSIONV4) {
            ...
            LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, pfrom->nVersion);
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                strprintf("Version must be %d or greater", HARDFORK_PROTOCOL_VERSIONV4));
            pfrom->fDisconnect = true;
            return false;
        }
        ...
        int64_t nLimitSince = nNow - 365 * 24 * 60 * 60;    // Ignore addresses older than a year
        BOOST_FOREACH (CAddress& addr, vAddr) {
            ...
            // Do not store addresses outside our network or not seen for long time
            if (fReachable && addr.nTime > nLimitSince)
...
{
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * GetStakeTargetSpacing(nNow) * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
