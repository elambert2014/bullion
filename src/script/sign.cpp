...
bool SignSignature(const CKeyStore &keystore, const CScript& fromPubKey, CMutableTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    LogPrintf("!!!!!!##########>> SignSignature called   ########!!!!!!!!!!\n");
    LogPrintf("!!!!!!##########   SignSignature called <<########!!!!!!!!!!\n");
...
// CBX2 function
bool SignSignature2(const CKeyStore &keystore, const CScript& fromPubKey, CTransaction& txTo, unsigned int nIn, int nHashType)
{
    assert(nIn < txTo.vin.size());
    CTxIn& txin = txTo.vin[nIn];
    ...
    // Leave out the signature from the hash, since a signature can't sign itself.
    // The checksig op will also drop the signatures from its hash.
    uint256 hash = SignatureHash2(fromPubKey, txTo, nIn, nHashType);
    ...
    if (whichType == TX_SCRIPTHASH)
    {
        ...
        // Recompute txn hash using subscript in place of scriptPubKey:
        uint256 hash2 = SignatureHash2(subscript, txTo, nIn, nHashType);
    ...
    // Test solution
    return VerifyScript2(txin.scriptSig, fromPubKey, txTo, nIn, true, 0);
}
...
// CBX2 function
bool SignSignature2(const CKeyStore &keystore, const CTransaction& txFrom, CTransaction& txTo, unsigned int nIn, int nHashType)
{
    assert(nIn < txTo.vin.size());
    CTxIn& txin = txTo.vin[nIn];
    assert(txin.prevout.n < txFrom.vout.size());
    ...
    return SignSignature2(keystore, txout.scriptPubKey, txTo, nIn, nHashType);
}
